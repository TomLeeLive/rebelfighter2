...
{		
	...
	//HDC hMaskMemDC = CreateCompatibleDC( m_hBackScreen->hDC);
	//HDC hColorMemDC = CreateCompatibleDC( m_hBackScreen->hDC);
	HBITMAP hOldMaskBitmap = (HBITMAP)SelectObject( hMaskMemDC, hMaskBitmap );
	HBITMAP hOldColorBitmap = (HBITMAP)SelectObject( hColorMemDC, hColorBitmap );
	
	//		BitBlt( m_hBackScreen->hDC,m_fPosX, m_fPosY,iWidth, iHeight, hMaskMemDC, 0, 0, SRCAND );
	//		BitBlt( m_hBackScreen->hDC,m_fPosX, m_fPosY,iWidth, iHeight, hColorMemDC, 0, 0, SRCINVERT );
	//		BitBlt( m_hBackScreen->hDC,m_fPosX, m_fPosY,iWidth, iHeight, hMaskMemDC, 0, 0, SRCINVERT );
	
	SelectObject( hMaskMemDC, hOldMaskBitmap );
	SelectObject( hColorMemDC, hOldColorBitmap );
	DeleteDC( hMaskMemDC );
